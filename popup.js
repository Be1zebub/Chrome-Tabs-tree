...
document.addEventListener("DOMContentLoaded", () => {
	...
	function AppendTab(tab, parent) {
		...
		if (tab.audible) {
			...
			img.src = speakerIcons[tab.mutedInfo.muted]
			audible = img
		}
				...
				const menu = ContextMenu([
					...
					},
					{
						...
						cback: () => {
							...
							audible.src = speakerIcons[tab.mutedInfo.muted]
						},
						customcheck: () => {
							return tab.audible
						},
	...
	function ContextMenu(items) {
		...
		items.forEach((item) => {
			if (item.customcheck && item.customcheck() == false) {
				return
			}
	...
	function SetContextMenuPosition(menu, e) {
		...
		// let x = Math.min(e.pageX, document.body.scrollWidth - w)
		// let y = Math.min(e.pageY, document.body.scrollHeight - h)
